...
namespace PxAbp.Tests
{
    public abstract class PxAbpTestBase : AbpIntegratedTestBase<PxAbpTestModule>
    {
        ...
        protected void UsingDbContext(Action<PxAbpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PxAbpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PxAbpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PxAbpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PxAbpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PxAbpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PxAbpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PxAbpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PxAbpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PxAbpDbContext, Task<T>> func)
        {
            T result;
